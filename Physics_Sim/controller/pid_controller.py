...
`PythonDroneController` is ALWAYS defined here, regardless of whether the DLL
is available -- this makes it possible to import it directly for
cross-validation against drone_bridge.DroneController even on a machine that
has a compiler. Importing plain `DroneController` from this module gives you
whichever backend is actually active (DLL if available, Python otherwise);
don't rely on that name specifically meaning "the Python one".
"""
...
# ---------------------------------------------------------------------------
# Pure-Python translation -- always defined (see module docstring)
# ---------------------------------------------------------------------------
import numpy as np
...
class PythonDroneController(BaseController):
    """
    ...
    def update(self, state: np.ndarray, dt: float) -> np.ndarray:
        ...
        # Position-loop timing accumulator. Mirrors controller.cpp's
        # dt_pos = now - _lastPositionMicros, which is an absolute-clock
        # read -- it keeps advancing in real time even while the safety
        # cut below is active, since Controller::update() returns before
        # ever touching _lastPositionMicros. Must accumulate here,
        # *before* the halt check, or a spell at >45 deg would silently
        # pause the position loop's clock instead of just skipping its
        # output, and the loop would fire later than it would in C++.
        self._pos_accum += dt
        ...
        # Inner attitude loop (AttitudeController::update). _cv_x/_cv_y
        # are world-frame (from the position loop); rotate into body
        # frame by -yaw before treating them as tilt commands. Mirrors
        # the Controller.cpp fix for the yaw-frame position bug: without
        # this, a keep_heading=False leg (nonzero target yaw) could
        # spiral away from the target once yaw isn't exactly 0.
        cos_psi, sin_psi = np.cos(psi), np.sin(psi)
        body_x =  self._cv_x * cos_psi + self._cv_y * sin_psi
        body_y = -self._cv_x * sin_psi + self._cv_y * cos_psi
        desired_pitch = body_x
        desired_roll  = -body_y    # positive left → negative roll
...
# ---------------------------------------------------------------------------
# DroneController resolves to whichever backend is actually active.
# ---------------------------------------------------------------------------
DroneController = drone_bridge.DroneController if _USING_DLL else PythonDroneController
